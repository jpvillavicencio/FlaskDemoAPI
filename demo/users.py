from flask import Blueprint, abort, request, make_response, jsonify, json
...
@auto.doc(groups=['users', 'public'])
def users_get(postcode=None):
    '''Get all users. If argument is added. Only gets users from that postcode.'''
    if postcode:
